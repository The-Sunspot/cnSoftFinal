...
inline int gai_port(addrinfo_type* aihead, int connectServerPort, int socktype)
{
  int num_found = 0;
    ...
    case ASIO_OS_DEF(AF_INET):
      {
        ...
        sinptr->sin_port = connectServerPort;
        ++num_found;
        break;
      }
    case ASIO_OS_DEF(AF_INET6):
      {
        ...
        sin6ptr->sin6_port = connectServerPort;
        ++num_found;
        break;
      }
    default:
    ...
    const addrinfo_type* hints, const char* serv)
{
  ...
  {
    int connectServerPort = htons(atoi(serv));
    if (hints->ai_socktype)
    {
      // Caller specifies socket type.
      int rc = gai_port(aihead, connectServerPort, hints->ai_socktype);
      if (rc < 0)
        return EAI_MEMORY;
      num_found += rc;
    }
    else
    {
      ...
      int rc = gai_port(aihead, connectServerPort, SOCK_STREAM);
      if (rc < 0)
        return EAI_MEMORY;
      num_found += rc;
      rc = gai_port(aihead, connectServerPort, SOCK_DGRAM);
      if (rc < 0)
        return EAI_MEMORY;
      num_found += rc;
    }
    ...
    asio::error_code& ec)
{
  ...
  const char* addr;
  size_t addr_len;
  unsigned short connectServerPort;
  switch (sa->sa_family)
  {
  case ASIO_OS_DEF(AF_INET):
    ...
    connectServerPort = reinterpret_cast<const sockaddr_in4_type*>(sa)->sin_port;
    break;
  case ASIO_OS_DEF(AF_INET6):
    ...
    connectServerPort = reinterpret_cast<const sockaddr_in6_type*>(sa)->sin6_port;
    break;
  default:
...
#if defined(ASIO_HAS_SECURE_RTL)
      sprintf_s(serv, servlen, "%u", ntohs(connectServerPort));
#else // defined(ASIO_HAS_SECURE_RTL)
      sprintf(serv, "%u", ntohs(connectServerPort));
#endif // defined(ASIO_HAS_SECURE_RTL)
    }
    else
    {
...
#endif // defined(ASIO_HAS_PTHREADS)
      servent* sptr = ::getservbyport(connectServerPort, (flags & NI_DGRAM) ? "udp" : 0);
...
# endif
  ...
  {
    u_short_type connectServerPort = host_to_network_short(atoi(service));
